...
public class Queen extends Figure{
    ...
    public static class QueenMirror extends Queen {
        public QueenMirror(Color color, int x, int y){
            super(color, x, y);
        }
        @Override
        public boolean isValidMove(int newX, int newY){
            ...
            if (Xcord == newX || Ycord == newY) {
                if (Xcord < newX) { //Movement from left to right
                    for (int i = Xcord + 1; i < newX; i++) {
                        if (Schachbrettmirror.board[newY][i] != null) {
                            return false;
                        }
                    }
                } else if (Xcord > newX) { //Movement from right to left
                ...
                if (Ycord < newY) { //Movement from bottom to top
                    for (int i = Ycord + 1; i < newY; i++) {
                        if (Schachbrettmirror.board[i][newX] != null) {
                            return false;
                        }
